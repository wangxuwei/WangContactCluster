;(function() {
    ...
    (function ($) {
      ...
      function showView(data) {
          ...
          var $canvas = $e.find(".clusterChartCanvas");
        ...
        $.each(childrenData, function(i, item) {
          ...
          if ((level - 1) > 0) {
            ...
            var newContainer = createContainer.call(view, newData, {
              x : cx,
              y : cy
            }, level - 1, (Math.PI + angle * i) + exAngle);
      ...
      function clickEvent(d) {
        ...
        app.ContactDao.getByName(d.target.name).done(function(userData) {
          ...
          var newContainer = createContainer.call(view, userData, {
            x : view.canvasW / 2,
            y : view.canvasH / 2
          }, view.level, (Math.PI + d.target.angleVal), true);
          ...
          if ($contactInfo.find("span").size() > 0) {
            ...
            createjs.Tween.get($contactInfo[0]).to({
              opacity : 0.1,
              left : leftVal,
              top : topVal
            }, app.animationSpeed, createjs.Ease.quartInOut).call(function() {
              $contactInfo.empty();
            });
          }
          ...
          createjs.Tween.get(statLayout).to({
            alpha : 0,
            x : ox,
            y : oy
          }, app.animationSpeed, createjs.Ease.quartInOut);
  
          createjs.Tween.get(newContainer).to({
            alpha : 1,
            x : (1 - view.scaleVal) * view.originPoint.x,
            y : (1 - view.scaleVal) * view.originPoint.y
          }, app.animationSpeed, createjs.Ease.quartInOut).call(function() {
